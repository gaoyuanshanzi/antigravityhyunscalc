...
function __probz(p) {
  if (typeof p !== "number" || isNaN(p) || p <= 0 || p >= 1) return NaN;
  return probit(1 - p);
}
...
function evaluateLine(rawLine) {
  ...
  const evaluator = new Function(
    "__log", "__average", "__sum", "__stdev_s", "__stdev_p", "__var_s", "__var_p", "__permut", "__combin", "factorial", "__zscore", "__zprob", "__probz",
    `return (${parsedExpr});`
  );
  const res = evaluator(
    __log, __average, __sum, __stdev_s, __stdev_p, __var_s, __var_p, __permut, __combin, factorial, __zscore, __zprob, __probz
